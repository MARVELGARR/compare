      ...
      },
      {
        ...
        cell: ({ row }) => (
            <div className="flex items-center gap-3">
                ...
                <div>
                    <div className="font-bold text-white text-base">{row.original.name}</div>
                    <div className="text-zinc-500 text-xs">{row.original.handle}</div>
                </div>
            </div>
        ),
      },
  ...
  return (
    <div className="w-full text-white font-sans mt-4">
                                ...
                                <>
                                    {MARKETS.map((m) => (
                                        <button 
                                            ...
                                            className={`px-3 py-1 rounded-full cursor-pointer transition-colors ${field.value === m.code ? 'bg-white text-black shadow-sm' : 'text-zinc-400 hover:text-white'}`}
                                        >
                                            {m.name}
                                        </button>
                                    ))}
      ...
      <div 
        ...
        className="rounded-md border border-zinc-800 overflow-hidden overflow-y-auto h-[calc(100vh-200px)] no-scrollbar min-h-[500px]"
      >
          <Table>
